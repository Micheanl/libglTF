...
class GltfInstance internal constructor(val handle: GltfHandle) {
    ...
    internal val geometryRenderers: Array<Array<Array<GltfGeometryRenderer>>> = createRenderers()
    internal val gpuSubmits: Array<Array<Array<Array<GpuSubmit>>>> = createGpuSubmits()
    ...
    private fun createRenderers(): Array<Array<Array<GltfGeometryRenderer>>> = Array(handle.asset.nodes.size) { nodeIndex ->
        val node = handle.asset.nodes[nodeIndex]
        val meshIndices = if (node.meshLods.isNotEmpty()) node.meshLods else intArrayOf(node.meshIndex)
        Array(meshIndices.size) { lod ->
            val meshIndex = meshIndices[lod]
    ...
    private fun createGpuSubmits(): Array<Array<Array<Array<GpuSubmit>>>> =
        ...
        Array(meshIndices.size) { lod ->
            ...
            if (meshIndex < 0) {
                ...
                val instanceCount = (node.instanceMatrices.size / 16).coerceAtLeast(1)
