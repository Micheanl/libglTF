...
class GpuBatch : AutoCloseable {
    ...
    private val paletteOffsets = HashMap<GltfInstance, Int>()
    ...
    private fun writeInstances(submits: List<GpuSubmit>, fromIndex: Int, toIndex: Int) {
        ...
        var paletteFloatCount = 0
        if (skinned) {
            for (index in fromIndex until toIndex) {
                val submit = submits[index]
                if (!paletteOffsets.containsKey(submit.instance)) {
                    paletteFloatCount += submit.instance.animationState.jointPalettes[submit.skinIndex].size
                }
            }
            ensurePaletteCapacity(paletteFloatCount * Float.SIZE_BYTES)
        }
    ...
    private fun writePalettes(submits: List<GpuSubmit>, fromIndex: Int, toIndex: Int) {
        val entries = paletteOffsets.entries
        if (entries.isEmpty()) return
        val skinIndex = submits[fromIndex].skinIndex
        val byteCount = entries.sumOf { (instance, _) ->
            instance.animationState.jointPalettes[skinIndex].size * Float.SIZE_BYTES
        }
        ...
        view.use { mapped ->
            ...
            for ((instance, offset) in entries) {
                data.position(offset * 4)
                data.put(instance.animationState.jointPalettes[skinIndex])
            }
        }
    }
