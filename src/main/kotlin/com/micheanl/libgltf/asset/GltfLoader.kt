...
import com.micheanl.libgltf.material.PbrSpecularGlossinessMaterial
...
object GltfLoader {
    ...
    private fun parseMaterials(root: JsonValue): Array<GltfMaterial> {
        ...
        return Array(values.size()) { index ->
            ...
            val pbrSpecGloss = JsonFields.value(extensions, "KHR_materials_pbrSpecularGlossiness")
            val baseColor = if (pbrSpecGloss != null) {
                JsonFields.floats(pbrSpecGloss, "diffuseFactor", floatArrayOf(1.0f, 1.0f, 1.0f, 1.0f))
            } else {
                JsonFields.floats(pbr, "baseColorFactor", floatArrayOf(1.0f, 1.0f, 1.0f, 1.0f))
            }
            val baseColorTex = if (pbrSpecGloss != null) {
                parseBinding(JsonFields.value(pbrSpecGloss, "diffuseTexture"))
            } else {
                parseBinding(JsonFields.value(pbr, "baseColorTexture"))
            }
            val pbrSpecGlossMaterial = pbrSpecGloss?.let {
                PbrSpecularGlossinessMaterial(
                    JsonFields.floats(it, "diffuseFactor", floatArrayOf(1.0f, 1.0f, 1.0f, 1.0f)),
                    parseBinding(JsonFields.value(it, "diffuseTexture")),
                    JsonFields.floats(it, "specularFactor", floatArrayOf(1.0f, 1.0f, 1.0f)),
                    parseBinding(JsonFields.value(it, "specularGlossinessTexture")),
                    JsonFields.float(it, "glossinessFactor", 1.0f)
                )
            }
            GltfMaterial(
    ...
    private fun parseNodes(root: JsonValue, decoder: AccessorDecoder): Array<GltfNode> {
        ...
        return Array(values.size()) { index ->
            ...
            val msftLod = JsonFields.value(JsonFields.value(node, "extensions"), "MSFT_lod")
            val meshLods = if (msftLod != null) JsonFields.ints(msftLod, "ids") else IntArray(0)
